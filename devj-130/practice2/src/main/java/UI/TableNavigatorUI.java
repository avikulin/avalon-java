...
public class TableNavigatorUI extends JFrame {
    ...
    private void initControls() {
        ...
        dataTable.setDefaultRenderer(DateTime.class, new DateTimeCellRenderer(false));
        dataHeaders = dataTable.getTableHeader();
    ...
    private boolean checkConnectionString(String s) {
        Matcher matcher = checkConnectionString.matcher(s);
        ...
        if (!checkConnectionString(s)) {
            JOptionPane.showMessageDialog(
                    ...
                    "Неправильный формат строки подключения.\n" +
                            ...
                            "Пример: jdbc:derby://localhost.company.com:1527/db1",
                    "Ошибка подключения",
                    JOptionPane.ERROR_MESSAGE);
            Tracer.get().logError(
                    this.getClass(),
